...
def upload_csv_to_gcs(local_file_path, bucket_name, destination_blob_name):
    try:
        ...
        project_id = "skilful-sensor-457808-v8"
...
lat_col = next((col for col in df.columns if col in ['latitude', 'lat']), None)
lon_col = next((col for col in df.columns if col in ['longitude', 'lng', 'lon', 'long']), None)

# Feature Engineering
df['age'] = 2025 - df['year_built']
features = ['bedrooms', 'bathrooms', 'sqft_living', 'sqft_lot', 'floors', 'zipcode', lat_col, lon_col, 'age']
X = df[features]
y = df['price']
...
# Display map of full dataset if location columns exist
if lat_col and lon_col:
    ...
    st.warning("Latitude/Longitude data not found in the dataset.")
