# Combined AVM Dashboard with Geospatial Data, Google Maps, Registration, and GCS Upload
...
# Load Data for Map and Model
data = load_data()
df = data.copy()
if not df.empty:
    df['age'] = 2025 - df['year_built']
    features = ['bedrooms', 'bathrooms', 'sqft_living', 'sqft_lot', 'floors', 'zipcode', 'latitude', 'longitude', 'age']
    X = df[features]
    y = df['price']
...
zipcode = st.sidebar.selectbox("Zipcode", sorted(df['zipcode'].unique())) if not df.empty else None
...
if latitude and longitude and model is not None:
